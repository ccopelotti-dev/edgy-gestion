...
async function main() {
  ...
  const numero = formatNumeroConPuntoVenta('0005', 7)
  const { y, color } = await dibujarEncabezadoConDatosFiscales(
    ...
    { letraFiscal: 'B', codigoAfip: 6, titular: 'Copelotti Marina Alejandra' },
  )
  doc.setFontSize(10)
  doc.setTextColor('#333333')
  doc.text('Cliente: Carlos Copelotti', 15, y + 5)
  dibujarPie(doc, empresa)
  const buf = Buffer.from(doc.output('arraybuffer'))
  fs.writeFileSync('/tmp/test_letra_fiscal.pdf', buf)
