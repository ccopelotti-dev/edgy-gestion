// ============================================================
// Modulo Home Keep — Pagos
// Edgy Gestion · Clon de compras/pages/OrdenesPago.tsx: arma un Pago con
// plazo, transferencia y cheques (uno o varios), y débito real en
// Tesorería al confirmar. En Home Keep esta tab se llama simplemente
// "Pagos" (no "Órdenes de Pago") en todos los textos visibles -- el
// modelo de datos interno sigue llamándose "pago", sin cambios.
// ============================================================
...
import { PagoDialog, ConfirmarPagoDialog, type CuentaBancariaOpcionDialog } from '../components/dialogs';
...
export default function Pagos() {
  ...
  const [proveedorNuevoPagoId, setProveedorNuevoPagoId] = useState('');
  ...
  const proveedorNuevoPago = proveedores.find((p) => p.id === proveedorNuevoPagoId);
  ...
  const handleSavePago = (data: {
    ...
    if (!proveedorNuevoPagoId) return;
    const now = nowISO();
    dispatch({
      type: 'ADD_PAGO',
      payload: {
        id: generarId(),
        proveedorId: proveedorNuevoPagoId,
        fecha: data.fecha,
        estado: 'pendiente',
        monto: data.monto,
    ...
    setProveedorNuevoPagoId('');
  };

  const handleConfirmarPago = (data: { fecha: string; lineasPago: Pago['lineasPago'] }) => {
  ...
  const handleAnular = (id: string) => {
    if (!confirm('¿Anular este pago? Todavía no se confirmó, así que no afectó ningún saldo ni cuenta.')) return;
  ...
  // Mismo patrón que Compras: PDF real como adjunto por WhatsApp vía el
  // agente, con respaldo a wa.me si falla.
  const handleEnviarWhatsapp = async (pago: Pago) => {
    const prov = proveedores.find((p) => p.id === pago.proveedorId);
    ...
    try {
      ...
      console.error('Home Keep · Pagos: no se pudo enviar por el agente, cae a wa.me', e);
  ...
  const estados: EstadoPago[] = ['pendiente', 'pagada', 'anulada'];
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <select
            className="rounded-lg border border-gray-300 py-2 px-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-gray-900/20"
            value={proveedorNuevoPagoId}
            onChange={(e) => setProveedorNuevoPagoId(e.target.value)}
          >
      ...
      {pagosFiltrados.length === 0 ? (
        <EmptyState
          ...
          title="No hay pagos"
          description="Elegí un proveedor y creá un pago para empezar."
        />
      ) : (
      ...
      {proveedorNuevoPago && (
        <PagoDialog
          ...
          proveedor={proveedorNuevoPago}
          comprobantesPendientes={comprobantesPendientes(proveedorNuevoPago.id)}
