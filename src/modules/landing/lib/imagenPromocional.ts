// ============================================================
// Generador de imagen promocional JPG -- reutilizado del módulo
// Productos y Stock (Fase 5b, ver
// productos-stock/lib/imagenPromocional.ts), a pedido explícito de
// Carlos ("en combo o promo ya habíamos desarrollado algo para esta
// finalidad, puede ser reutilizable"): es la "Capa 1" del plan de
// redes -- generar contenido listo para compartir, sin ninguna
// integración con Instagram/WhatsApp (eso queda 100% a criterio del
// cliente, ver charla de Fase 79).
//
// Es una copia deliberada, no un import cruzado entre módulos: mismo
// criterio que ya usa este módulo con lib/imagenes.ts (mirror de
// productos-stock/lib/imagenes.ts) -- cada módulo queda autocontenido
// para no acoplar su carga lazy a la de otro módulo. La única
// diferencia con el original es de nombres (ya no es exclusivo de
// combos: sirve igual para productos, que no tienen `etiqueta` -- ese
// campo queda simplemente sin usarse en ese caso, el generador ya lo
// contempla como opcional).
//
// Compone, 100% client-side con la Canvas API del navegador (sin
// dependencias nuevas), un flyer rectangular vertical (1080x1620,
// pensado para historias/redes) con: banda de logo + etiqueta
// (posición configurable), foto(s), nombre, precio bien resaltado y
// descripción.
//
// Si no hay foto o el logo no carga (ej. bloqueo CORS de un
// bucket/host de terceros), se degrada con elegancia a una plantilla
// con el espacio en blanco/color sólido en vez de romper la
// generación.
// ============================================================
...
export interface DatosImagenPromocional {
  nombre: string
  precio: number
  descripcion?: string
  /** Etiqueta/badge (ej. "Imperdible", "Promo del mes"). Solo tiene sentido
   * para combos/promos -- un producto normal simplemente no manda este
   * campo y el flyer queda sin badge. */
  etiqueta?: string
  /** Foto(s) elegidas de la galería -- 1 (ocupa todo el ancho) o 2 (se
   * reparten según `layout`, estilo "pantalla dividida"). Si viene vacío
...
// Rectángulo vertical, pensado para historias/estados (1080x1620).
const ANCHO = 1080
 ...
 * genérica sans-serif, para la etiqueta/badge del flyer. */
...
/**
 * Genera la imagen promocional (producto o combo) y devuelve un data URL
 * JPEG listo para mostrar en un <img> o descargar.
 */
export async function generarImagenPromocional(
  datos: DatosImagenPromocional,
): Promise<string> {
