...
import { CatalogoTecnico, type CatalogoTecnicoHandle } from './CatalogoTecnico'
...
export function ProductoDialog({
  ...
  // Fase 48e: Catálogo Técnico -- mismo componente compartido que
  // InsumoDialog, ver CatalogoTecnico.tsx.
  const { clienteId } = useClienteId()
  const catalogoTecnicoRef = useRef<CatalogoTecnicoHandle>(null)
  ...
  function handleCancelar() {
    ...
    subidasEnEstaSesionRef.current = new Set()
    catalogoTecnicoRef.current?.limpiarSesion()
    onOpenChange(false)
  }
    ...
    >
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid gap-4 py-4">
          ...
          {/* Fase 48e: Catálogo Técnico -- componente compartido con
              InsumoDialog, ver CatalogoTecnico.tsx. No se mezcla con la
              galería de arriba: esa es pública/catálogo, esto es un
              repositorio interno (manual, ficha técnica, garantía). */}
          <CatalogoTecnico
            ref={catalogoTecnicoRef}
            open={open}
            documentos={form.documentos}
            onChange={(documentos) => update('documentos', documentos)}
            clienteId={clienteId}
            descripcionAyuda="Manual de instalación, ficha técnica del fabricante, certificado de garantía o video de uso de este producto -- quedan a mano acá para consultarlos rápido (y, a futuro, para que un agente de IA los use como referencia)."
          />
...
export function InsumoDialog({
  ...
  // Fase 48c/48e: Catálogo Técnico -- documentos del insumo (PDF/imagen
  // vía bucket privado "archivos-cliente", link de video, o texto libre).
  // Componente compartido con ProductoDialog -- ver CatalogoTecnico.tsx.
  const { clienteId } = useClienteId()
  const catalogoTecnicoRef = useRef<CatalogoTecnicoHandle>(null)
    ...
    >
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid gap-4 py-4">
          ...
          {/* Fase 48c/48e: Catálogo Técnico -- componente compartido con
              ProductoDialog, ver CatalogoTecnico.tsx. */}
          <CatalogoTecnico
            ref={catalogoTecnicoRef}
            open={open}
            documentos={form.documentos}
            onChange={(documentos) => update('documentos', documentos)}
            clienteId={clienteId}
            descripcionAyuda="Fichas técnicas, hojas de seguridad, instructivos de dosificación o videos de uso de este insumo -- quedan a mano acá para consultarlos rápido (y, a futuro, para que un agente de IA los use como referencia)."
          />
