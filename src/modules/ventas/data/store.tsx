...
function presupuestoItemToRow(item: PresupuestoItem, presupuestoId: string) {
  return {
    ...
    variante_servicio_id: item.varianteServicioId ?? null,
    descripcion: item.descripcion,
    cantidad: item.cantidad,
...
function syncToSupabase(action: VentasAction, nextState: VentasState, clienteId: string) {
  switch (action.type) {
    ...
    case 'ADD_PRESUPUESTO': {
      ...
      // el INSERT del presupuesto haya confirmado en Supabase (encadenado
      // con .then, no en paralelo). Antes ambos INSERT se disparaban al
      // mismo tiempo, y la política RLS de `presupuesto_items` (que exige
      // que exista una fila en `presupuestos` con ese id) podía evaluarse
      // antes de que la fila padre estuviera confirmada, devolviendo 42501
      // (RLS) y perdiendo los items en silencio.
      supabase
        .from('presupuestos')
        .insert(presupuestoToRow(p, clienteId))
        .then((res) => {
          logErr('alta de presupuesto')(res);
          if (!res.error && p.items.length) {
            supabase.from('presupuesto_items').insert(p.items.map((i) => presupuestoItemToRow(i, p.id))).then(logErr('items de presupuesto'));
          }
        });
      return;
    }
    ...
    case 'CONVERTIR_PRESUPUESTO_A_ORDEN': {
      ...
      if (orden) {
        // Mismo fix que ADD_PRESUPUESTO: encadenar el INSERT de items
        // después del INSERT de la orden para evitar la carrera RLS.
        supabase
          .from('ordenes_venta')
          .insert(ordenToRow(orden, clienteId))
          .then((res) => {
            logErr('alta de orden (desde presupuesto)')(res);
            if (!res.error && orden.items.length) {
              supabase.from('orden_venta_items').insert(orden.items.map((i) => ordenItemToRow(i, orden.id))).then(logErr('items de orden'));
            }
          });
      }
      return;
    ...
    case 'ADD_ORDEN': {
      ...
      supabase
        .from('ordenes_venta')
        .insert(ordenToRow(o, clienteId))
        .then((res) => {
          logErr('alta de orden')(res);
          if (!res.error && o.items.length) {
            supabase.from('orden_venta_items').insert(o.items.map((i) => ordenItemToRow(i, o.id))).then(logErr('items de orden'));
          }
        });
      return;
    }
    ...
    case 'ADD_COMPROBANTE': {
      ...
      if (!c) return;
      // Mismo fix: los items del comprobante se insertan recién cuando el
      // INSERT del comprobante confirmó, para que la política RLS de
      // `comprobante_venta_items` encuentre la fila padre ya visible.
      supabase
        ...
        .then((res) => {
          ...
          if (!res.error && c.items.length) {
            supabase.from('comprobante_venta_items').insert(c.items.map((i) => comprobanteItemToRow(i, c.id))).then(logErr('items de comprobante'));
          }
        });
    ...
    case 'ADD_COBRO': {
      ...
      if (!cobro) return;
      // Mismo fix: las imputaciones referencian al cobro por id, así que
      // se encadenan después de que el INSERT del cobro haya confirmado.
      supabase
        .from('cobros')
        .insert(cobroToRow(cobro, clienteId))
        .then((res) => {
          logErr('alta de cobro')(res);
          if (!res.error && cobro.imputaciones.length) {
            supabase
...
function presupuestoItemFromRow(r: any): PresupuestoItem {
  return {
    ...
    varianteServicioId: r.variante_servicio_id ?? undefined,
